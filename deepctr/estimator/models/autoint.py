        ...
        with variable_scope(DNN_SCOPE_NAME):
            ...
            if len(dnn_hidden_units) > 0 and att_layer_num > 0:  # Deep & Interacting Layer
                ...
                final_logit = tf.keras.layers.Dense(
                    1, use_bias=False, kernel_initializer=tf.compat.v1.keras.initializers.glorot_normal(seed))(stack_out)
            elif len(dnn_hidden_units) > 0:  # Only Deep
                deep_out = DNN(dnn_hidden_units, dnn_activation, l2_reg_dnn, dnn_dropout, dnn_use_bn, seed=seed)(dnn_input, training=train_flag)
                final_logit = tf.keras.layers.Dense(
                    1, use_bias=False, kernel_initializer=tf.compat.v1.keras.initializers.glorot_normal(seed))(deep_out)
            elif att_layer_num > 0:  # Only Interacting Layer
                final_logit = tf.keras.layers.Dense(
                    1, use_bias=False, kernel_initializer=tf.compat.v1.keras.initializers.glorot_normal(seed))(att_output)
            else:  # Error
